...
Pre-cleans rows and adds closest-valid-adj close ordinal column
...
def _build_closest_adj_column(rows: List[Dict[str, Any]]) -> None:
    ...
    Walks backwards so we always know the nearest earlier date with valid adj close.
    Uses the column name 'adj close' (with a space) as generated by simulate_walk.
    """
    last_good_ord: Optional[int] = None
    for row in reversed(rows):
        # The correct column name is 'adj close' (with a space)
...
def get_closest(req: ClosestRequest):
    ...
    # --- fast path for adj close (note: column name is 'adj close' with space) ---
    if field == "adj close":
        start_row = stock_map.get(tgt_d)
        if start_row and start_row.get("adj close") is not None:
            return {"symbol": sym,
                    "requested_date": tgt_d,
                    "found_date": tgt_d,
                    "value": start_row["adj close"]}
